...
import { fresnel } from '../../../modules/glsl/lighting';
import { rotate } from '../../../modules/glsl/math';
import { combinations, sdf } from '../../../modules/glsl/raymarching';
import { AMOUNT, datas } from './store';
...
float sdf(vec3 p) {
  ...
  float angle = PI / 2.0;
	...
	vec3 rp = rotate(tp, u_rotations[0], angle);
  float final = sdBox(rp, vec3(0.15)) - 0.03;

  for(int i = 1; i < MaxCount; i++) {
		tp = p + -u_positions[i] * correct;
    rp = rotate(tp, u_rotations[i], angle);
